...
/* constructor */
NFA::NFA(){
	id=nstate_nr++; //last instantiated nfa state

    //nstate==0xFFFFFFFF==64bit1
	if (nstate_nr==NO_STATE) fatal("NFA:: Max number of states reached");

	transitions=new pair_set();//labeled transitions set<pair<symbol_t,NFA*>*>
	epsilon=new nfa_list(); //epsilon transitions
	...
	delete_only_me=0;//to avoid recursion during deletion

	depth=UNDEFINED;//upper bound of state


	first=this;//first state
	last=this;//last state
}
...
bool NFA::has_transition(symbol_t c, NFA *nfa){
	FOREACH_TRANSITION(it){//use iterator check transition exist?
...
int NFA::reduce(){
	...
	remove_duplicates();//remove duplicate states
	reset_state_id();//reset remain states id
	// contains mapping between DFA and NFA set of states
